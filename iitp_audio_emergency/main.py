...
from utils import select_optimizer, evaluate, train
...
def train_process(args, model, train_loader, test_loader, optimizer, criterion, device):
    ...
    for epoch in range(args.num_epoch):
        model.train()
        train_loss, train_iou = train(model=model, train_loader=train_loader, optimizer=optimizer,
                                      criterion=criterion, device=device, epoch=epoch, total_epochs=args.num_epoch)
        model.eval()
        test_loss, test_iou = evaluate(model=model, test_loader=test_loader,
                                                device=device, criterion=criterion)

        report_dict = dict()
        report_dict["train__loss"] = train_loss
        report_dict["train__iou"] = train_iou
        report_dict["test__loss"] = test_loss
        report_dict["test__iou"] = test_iou
...
def main():
    # Argument Settings
    parser = argparse.ArgumentParser(description='Audio Threat Detection')
    ...
    trainset = AudioDataset(data_frame=df[:train_size], root_dir=f'{DATASET_PATH}/train/train_data')
    testset = AudioDataset(data_frame=df[train_size:], root_dir=f'{DATASET_PATH}/train/train_data')
    ...
    if args.mode == 'train':
        ...
        test_loss, test_iou = evaluate(model=model, test_loader=test_loader, device=device, criterion=criterion)
        logger.info(test_loss, test_iou)
